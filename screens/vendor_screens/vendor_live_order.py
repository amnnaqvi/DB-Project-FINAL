...
from PyQt6.QtWidgets import QMainWindow, QLabel, QInputDialog, QMessageBox, QComboBox, QPushButton, QScrollArea, QVBoxLayout, QHBoxLayout, QWidget, QFrame, QGridLayout
import sys
from pathlib import Path
sys.path.append(str(Path(__file__).resolve().parent.parent.parent))
from screens.database import DatabaseManager, get_db_connection

class View_Live_Orders(QMainWindow):
    def __init__(self, ID, name):
        super().__init__()
        self.setWindowTitle("Live Orders")
        ...
        # Title Label
        self.titleLabel = QLabel("Live Orders", self.centralwidget)
    ...
    def fetch_live_orders(self):
        live_orders=self.db_manager.get_live_orders(self.vendor_id)
        print (live_orders)
        return [{'id':order[0],'order':order[1],'user':order[2],'quantity':order[3],'price':order[4],'placed':order[5]} for order in live_orders]
    ...
    def populate_order_grid(self):
        ...
        live_orders_list= self.fetch_live_orders()
        display_list=[]
        for order1 in live_orders_list:
            order_dict={
                'order_id':order1['id'], 
                'user_id':order1['user'], 
        ...
        for row, order in enumerate(self.filtered_orders[:]):
            ...
            order_id_label = QLabel(f"Order ID: {str(order['order_id'])}", self.grid_content)
            ...
            dropdown.addItems(['Pending','Making', 'Ready', 'Completed'])  # Add options to dropdown
            current_status = self.db_manager.fetch_current_order_status(str(order['order_id']))
    ...
    def status_update(self,order,text):
        QMessageBox.information(self, "Status Update Confirmed", f"Status of Order {str(order['order_id'])} for User {order['user_id']} is now {text}"
...
def main():
    ...
    window = View_Live_Orders(101, "John Doe")
    window.show()
    sys.exit(app.exec())
